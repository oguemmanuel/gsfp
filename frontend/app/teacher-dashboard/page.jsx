...
const TeacherDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/dashboard/teacher", {
        credentials: "include",
      })
  ...
  const fetchSurveyQuestions = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/survey-questions/teacher", {
        credentials: "include",
      })
  ...
  const handleSurveySubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/submit-survey/teacher", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">Total in school</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{stats.studentsPresent || 0}</p>
                <p className="text-xs text-gray-500 mt-1">{stats.attendanceRate || 0}% attendance</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{stats.mealsServed || 0}</p>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">From this school</p>
              </div>
        ...
        {/* Program Impact - Only Real Data */}
        {(stats.studentsEnrolled > 0 || stats.mealsServed > 0) && (
          <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">School Feeding Program Impact</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center p-4 bg-blue-50 rounded-lg">
                <div className="text-2xl font-bold text-blue-600">{stats.attendanceRate || 0}%</div>
                <div className="text-sm text-blue-600">Daily Attendance Rate</div>
              </div>
              <div className="text-center p-4 bg-green-50 rounded-lg">
                <div className="text-2xl font-bold text-green-600">{stats.surveyResponses || 0}</div>
                <div className="text-sm text-green-600">Teacher Survey Responses</div>
              </div>
              <div className="text-center p-4 bg-orange-50 rounded-lg">
                <div className="text-2xl font-bold text-orange-600">
                  {stats.studentsPresent > 0 ? Math.round((stats.mealsServed / stats.studentsPresent) * 100) : 0}%
                </div>
                <div className="text-sm text-orange-600">Meal Participation Rate</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-semibold text-gray-900">Teacher Survey</h3>
